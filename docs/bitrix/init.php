...
const BITRIX_TG_DEBUG_LAST_PAYLOAD = false;
...
function bitrixTgBuildPayload(int $elementId, string $action): ?array
{
    ...
    $result = \CIBlockElement::GetList([], ["ID" => $elementId, "CHECK_PERMISSIONS" => "N"], false, false, $select);
    $fields = $result->Fetch();
    if (!$fields) {
        return null;
    }
    ...
    $section = bitrixTgSectionInfo($fields["IBLOCK_SECTION_ID"] ?? null);
    ...
    $telegram = bitrixTgFindProperty(
        $properties,
        ["publish_telegram", "telegram_publish", "pub_news_telegram", "pub_news_tg", "publish_to_telegram"],
        ["Опубликовать в Telegram", "Telegram"]
    );
    $vk = bitrixTgFindProperty(
        $properties,
        ["publish_vk", "vk_publish", "pub_news_vk", "pub_news_vkpost", "publish_to_vk", "publish_vkontakte"],
    ...
    return [
        ...
        "iblock_section_name" => (string)($section["name"] ?? ""),
        ...
        "post_type" => $postTypeRaw !== "" ? $postTypeRaw : "unknown",
...
function bitrixTgSectionInfo($sectionId): ?array
{
    $id = (int)$sectionId;
    if ($id <= 0) {
        return null;
